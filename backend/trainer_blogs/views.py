...
def get_posts(request):
    trainer = get_object_or_404(ClientTrainer, client = request.user.id)
    posts = TrainerBlog.objects.filter(user = trainer.trainer.id)
...
def update_post(request,pk):
        post = get_object_or_404(TrainerBlog, pk=pk)
